...
if ($session->is_logged_in() && $session->object_type == 5){
	...
	if (isset($_GET['personnelid'])){
		...
		$msg_obj->add_error("No Bus Personnel ID provided to view.");		
	}
	...
	if (isset($_POST['submit'])){
		...
		if ($bus_personnel_to_read_update->update()){
			$msg_obj->add_confirmation("Success! The Bus Personnel details were updated. ");			
		} else {
			$msg_obj->add_error("Error! The Bus details could not be updated. ");
		}
	}
	...
	if (isset($_POST['assign'])){
		...
		if ($buses_bus_personnel_to_read_update->create()){
			$msg_obj->add_confirmation("Success! The Bus Personnel was assigned to the given Bus. ");			
		} else {
			$msg_obj->add_error("Error! The Bus Personnel was not assigned to the given Bus. ");
		}
	}
	...
	if (isset($_POST['update'])){
		if ($_POST['old_password'] == $bus_personnel_to_read_update->password) {
			...
			if ($bus_personnel_to_read_update->update()){
				$msg_obj->add_confirmation("Success! The user's password was updated. ");				
			} else {
				$msg_obj->add_error("Error! The user's password could not be updated. ");
			}
    ...
    <div id="wrap">
      ...
      <header class="jumbotron subhead">
         ...
         if (!empty($profile_picture_of_bus_personnel->filename)) {
         	...
         	echo '<img src="../img/default-prof-pic.jpg" style="width: 100px; height: 200px;" class="img-rounded pull-right" alt="Please upload a profile picture" />';
         } 
         ?>
