...
if ($session->is_logged_in() && $session->object_type == 5){
	...
	if (isset($_GET['adminid'])){
		...
		$profile_picture_of_other_admin_users = $photo->get_profile_picture($session->object_type, $user_to_read_update->id);
	} else {
		$msg_obj->add_error("No Admin ID provided to view.");		
	}
	...
	if (isset($_POST['submit'])){
		...
		if ($user_to_read_update->update()){
			$msg_obj->add_confirmation("Success! The user details were updated. ");			
		} else {
			$msg_obj->add_error("Error! The user details could not be updated. ");
		}
	}
    ...
    <div id="wrap">
      ...
      <header class="jumbotron subhead">
         ...
         if (!empty($profile_picture_of_other_admin_users->filename)) {
         	echo '<img src="../../' . $profile_picture_of_other_admin_users->image_path() . '"style="width: 200px; height: 200px;" class="img-rounded pull-right" />'; 
         } else {
